...
LogDebug("msSemanticFrame",msSemanticFrame, "-->\r\n");
            break;
        }
    }
}
...
LogDebug("MSSFDecode(msSemanticFrame)", msSemanticFrame, '\r\n');
...
LogDebug("alteredquery", alteredquery, '\r\n');
...
LogDebug("285", constraintListTemp, '\r\n');
            var constraintSet = {};
...
LogDebug("entityTerm", termArray[j], '\r\n');
            filterTermDict[code] = 1;
        }
    }
...
LogDebug("res", i, res[i].replace(/word:\(([^\)]+)\)/g, "$1"), '\r\n');
		}
	}
LogDebug("459", '\r\n');	
...
LogDebug("matchData.wordFoundTitleArray", matchData.wordFoundTitleArray, '\r\n');
            var isCandidateMatchSnippet = WordsFoundForTitleSnippet(curCandTerm, matchData.wordFoundBodyArray, matchData.snippet) == 1;
			/*if(!isCandidateMatchTitle || !isCandidateMatchSnippet){
				code = GenerateHashCode(curCandTerm);
				var termArray = new Array();
				if(code in canonicalTermDic){
					termArray = canonicalTermDic[code];
				}
				for(k = 1; k < termArray.length; k++) {
					curCandTerm = termArray[k];
					if(!isCandidateMatchTitle){
						isCandidateMatchTitle = WordsFoundForTitleSnippet(curCandTerm, matchData.wordFoundTitleArray, matchData.title) == 1;
					}
					if(!isCandidateMatchSnippet){
						isCandidateMatchSnippet = WordsFoundForTitleSnippet(curCandTerm, matchData.wordFoundBodyArray, matchData.snippet) == 1;
					}
				}
			}*/
LogDebug("matchData.title and snippet", matchData.title, '\r\n', 'snippet', matchData.snippet, '\r\n');
LogDebug("isCandidateMatchTitle&Snippet",matchData.url, curCandTerm, isCandidateMatchTitle, isCandidateMatchSnippet, '\r\n');
			if (isCandidateMatchTitle) {
                if (i < 5) {
...
//LogDebug("[i].original", i, constraintList[i].original, '\r\n') ;
...
//LogDebug("isConsOriMatchTitle&Url", isConsOriMatchTitle, isConsOriMatchUrl, '\r\n');
    //Verify whether is condition0 match
...
//LogDebug("1944", url, top5IntentMatchSum, top5EntityIntentASCount, top8EntityIntentASCount, '\r\n');
...
//LogDebug("1965", url, top5ConstraintMatchSum, '\r\n');
...
LogDebug("canonicalQuery", canonicalQuery, '\r\n');
...
LogDebug("oriPos of rankScoreVector_15th", oriPos,drScoreThreshold , '\r\n');
    var maxDRScorePos = 0;
...
//LogDebug("IsGuard", i, documents[i].url, '1', '\r\n');
            rankVectorInOriPlace.push({ index: i });
        }
        else {
//LogDebug("IsGuard", i, documents[i].url, '0', '\r\n');
...
//LogDebug("RankerCondition1", '1', i, documents[documentPosition].url, '\r\n');
...
LogDebug("RankerCondition2", "1", url, documentPosition, '\r\n');
...
LogDebug("RankerCondition2", "0", url, documentPosition, '\r\n');
                rankVector[i].signal = 100 + (0.95 - i / 20.0);
            }
        }
        else {
LogDebug("RankerCondition1", '0', i, documents[documentPosition].url, '\r\n');
...
//LogDebug("posby0", posby0, '\r\n');
	...
	for(var j = 0; j < top20doc; ++j){
		...
		LogDebug("adjustRankerVector", documents[idx].url, adjustRankerVector[j].signal, '\r\n');
	}
...
LogDebug("documentScore", documents[idx].url, documents[idx].score, '\r\n');
    }
...
//LogDebug("2387", url, constraintMatchCondition, MSSFDecodeResult.constraint, '\r\n', matchData, authorityScore, constraintMatchUrlScore, i, '\r\n');
...
//LogDebug("documentPosition&maxDRScorePos", documentPosition, maxDRScorePos,'\r\n');
    ...
    var con2 = false;
if (documentPosition < 5) {
//LogDebug('2408', documents[documentPosition].url, ';', 'guardingScore', keyFeatures.guardingScore, ';', 'isSiteConsMatchDomain', isSiteConsMatchDomain, ';', 'lowQualitySiteScore', keyFeatures.lowQualitySiteScore, ';', 'entityMatchScore', keyFeatures.entityMatchScore, ';', 'intentMatchScore', keyFeatures.intentMatchScore, ';', 'constraintMatchScore', keyFeatures.constraintMatchScore,';', '\r\n');
}
