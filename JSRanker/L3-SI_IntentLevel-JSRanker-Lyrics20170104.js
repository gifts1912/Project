...
function MainRanker(subIntentId, subIntentScore, MSSFDecodeResult, matchDataArray, intentMatchCondition, constraintMatchCondition, documentsLocal, documentCount) {
  ...
  var entityMatchScore, topSiteScoreResult, authorityScore, specificSiteScore, intentMatchScore;
	for (i = 0; i < top20doc; ++i) {
		curDoc = documentsLocal[i];
		if (i === 0){
			matchData = matchDataArray[i];
  ...
  var documentPosition;
	for (i = 0; i < top20doc; i++) {
		documentPosition = i;
		if (!IsGuard(keyFeaturesOfDocuments[i], documentsLocal[i].rerankfeatures, i, entityMatchThreshold, maxDRScorePos, isSiteConsMatchDomain, sumIntentMatchTop5)) { 
